...
仓库已包含教材 `虚拟地理环境实践教程 0922版本.docx`，但不会提交生成后的 `index/kb.pkl`。应用启动时会检查该文件：如果不存在，会自动解析教材、按章节切分文本、使用 sklearn TF-IDF 构建索引并保存到 `index/kb.pkl`；如果构建失败，则进入纯 LLM 模式，不会导致应用崩溃。

首次部署或冷启动通常需要等待约 2–8 分钟，其中依赖安装时间占主要部分；教材索引本身在常规 CPU 环境中约需几十秒。验证方法：打开应用后询问“土地利用方向有什么可以使用的模型？”，正常情况下应命中城市土地利用变化模拟章节，并回答 PLUS、LEAS、CARS 等内容。也可以查看启动日志，确认出现索引片段数量且没有“知识库未加载”错误。
